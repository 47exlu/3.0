...
export const useRapperGame = create<RapperGameStore>()(
  subscribeWithSelector((set, get) => ({
    ...
    advanceWeek: () => {
      ...
      // Use the direct platform stream map created from songs as our source of truth
      // This is critical - we need to use the song-level streams as they're being tracked correctly
      
      // Initialize platform totals with current values to preserve accumulated streams
      const platformTotalStreams: Record<string, number> = {};
      
      // Initialize with current totals from platforms
      currentState.streamingPlatforms.forEach(platform => {
        platformTotalStreams[platform.name] = platform.totalStreams || 0;
      });
      
      // Add new streams from platformStreamMap (calculated from songs)
      Object.entries(platformStreamMap).forEach(([platformName, newStreams]) => {
        if (!platformTotalStreams[platformName]) {
          platformTotalStreams[platformName] = 0;
        }
        // Add the new streams from songs to the running total
        platformTotalStreams[platformName] += newStreams;
        
        console.log(`Adding ${newStreams} streams to platform ${platformName}, new total: ${platformTotalStreams[platformName]}`);
      });
      
      // Now calculate NEW streams this week for each platform
      const newWeeklyStreamsByPlatform: Record<string, number> = {};
      
      // For each platform, calculate the difference between current and previous week
      currentState.streamingPlatforms.forEach(platform => {
        const platformName = platform.name;
        const currentTotal = platformTotalStreams[platformName] || 0;
        const previousTotal = platform.totalStreams || 0;
        
        // Calculate growth (ensure it's not negative)
        const growth = Math.max(0, currentTotal - previousTotal);
        
        // Store the weekly growth for this platform
        newWeeklyStreamsByPlatform[platformName] = growth;
        
        console.log(`DEBUG: Platform ${platformName} - Previous total: ${previousTotal}, Current total: ${currentTotal}, Growth: ${growth}`);
      });
      
      // We don't need updatedPlatformStreams anymore since we're using platformTotalStreams directly
      
      // Now update the streaming platforms with these totals
      updatedState.streamingPlatforms = updatedState.streamingPlatforms.map(platform => {
        const platformName = platform.name;
        // Use the pre-calculated total streams from summing all album streams
        const platformTotal = platformTotalStreams[platformName] || 0;
        const newWeeklyStreams = newWeeklyStreamsByPlatform[platformName] || 0;
        
        // Use the pre-calculated new streams
        const newStreams = Math.max(0, newWeeklyStreams); // Make sure it's never negative
        
        // Calculate revenue from new streams
        const platformRevenue = calculateStreamingRevenue(newStreams, platformName);
        
        console.log(`Platform ${platformName} - New streams: ${newStreams}, Revenue generated: $${platformRevenue.toFixed(2)}`);
        
        // Return updated platform
        return {
          ...platform,
          totalStreams: platformTotal, // Update with accurate total from album streams
          revenue: platform.revenue + platformRevenue
        };
      });
      
      // Calculate total streams across all platforms for career level and weekly stats
      // This is critical for accurate career analytics 
      const allStreams = (updatedState.streamingPlatforms || [])
        .reduce((total, platform) => total + platform.totalStreams, 0);
      
      // Log to verify total streams
      console.log(`Total streams across all platforms: ${allStreams.toLocaleString()}`);
      
      // Make sure this is reflected in songs too to keep in sync
      const totalSongStreams = (updatedState.songs || [])
        .filter(song => song.released)
        .reduce((total, song) => total + song.streams, 0);
      
      console.log(`Total streams across all songs: ${totalSongStreams.toLocaleString()}`);
      
      // Verify that we're counting all platform streams as well
      const totalPlatformStreams = Object.values(platformTotalStreams)
        .reduce((sum, val) => sum + val, 0);
      
      console.log(`Total streams from platformTotalStreams: ${totalPlatformStreams.toLocaleString()}`);
      
      
      // Calculate this week's revenue from all platforms using the new weekly streams from each platform
      const weeklyRevenue = (updatedState.streamingPlatforms || [])
        .reduce((total, platform) => {
          // Get the calculated new streams from the newWeeklyStreamsByPlatform object
          const newStreams = newWeeklyStreamsByPlatform[platform.name] || 0;
          
          // Ensure newStreams is never negative
          const validNewStreams = Math.max(0, newStreams);
          
          // Calculate revenue from new streams using platform-specific rates
          const platformRevenue = calculateStreamingRevenue(validNewStreams, platform.name);
      ...
      // Generate random events with player stats to filter for requirements
      const randomEvent = getRandomEventForWeek(
        currentState.activeRandomEvents,
        currentState.resolvedRandomEvents.map(e => e.id),
        currentState.stats // Pass player stats to check requirements
      );
      ...
      currentState.aiRappers.forEach(rapper => {
        ...
        if (Math.random() < requestChance) {
          ...
          newCollaborationRequests.push({
            id: `feat-req-${rapper.id}-${newWeek}`,
            fromRapperId: rapper.id,
            tier: requestTier,
            weekRequested: newWeek,
            status: "pending",
            expiresWeek: newWeek + 4 // Request expires after 4 weeks
          });
        }
      });
    ...
    postOnSocialMedia: (platformName, content = "", images = []) => {
      ...
      // Create a new post with appropriate structure
      const now = new Date();
      const postId = uuidv4();
      ...
      let viralStatus: ViralStatus = "not_viral";
      let viralMultiplier = 1;
      let followerGain = 20; // Base follower gain for any post (even non-viral)
      let reputationGain = 1; // Small reputation gain for any post
      let wealthGain = 0;
      
      // Calculate viral chance based on reputation, marketing, and trend effect
      const reputationFactor = currentState.stats.reputation / 100;
      const marketingFactor = currentState.stats.marketing / 100;
      const platformTrendFactor = get().getTrendEffect(platformName); // Function to calculate trend effect
      
      // Calculate base viral chance (higher for TikTok to encourage its use)
      let baseViralChance = 0.1;
      if (platformName === "TikTok") baseViralChance = 0.15;
      
      const viralChance = baseViralChance * (reputationFactor + marketingFactor + platformTrendFactor) / 3;
      const superViralChance = viralChance * 0.2; // 20% chance of super viral if already viral
      
      // Determine if post goes viral
      const randomValue = Math.random();
      console.log("Post chance values:", { randomValue, superViralChance, viralChance });
      
      if (randomValue < superViralChance) {
        viralStatus = "super_viral";
        viralMultiplier = 5;
        followerGain = Math.floor(50 + Math.random() * 100 * viralMultiplier);
        reputationGain = Math.floor(2 + Math.random() * 3 * viralMultiplier);
        wealthGain = Math.floor(50 + Math.random() * 100 * viralMultiplier);
      } else if (randomValue < viralChance) {
        viralStatus = "viral";
        viralMultiplier = 2;
        followerGain = Math.floor(20 + Math.random() * 50 * viralMultiplier);
        reputationGain = Math.floor(1 + Math.random() * 2 * viralMultiplier);
        wealthGain = Math.floor(20 + Math.random() * 50 * viralMultiplier);
      } 
      
      console.log("Post outcome:", { 
        viralStatus, 
        viralMultiplier, 
        followerGain, 
        reputationGain, 
      ...
      let post: SocialMediaPost = {
        ...
        date: now,
        postWeek: currentState.currentWeek,
        likes: Math.floor(platform.followers * 0.05 * postEngagement * viralMultiplier) + Math.floor(Math.random() * 20),
        comments: Math.floor(platform.followers * 0.01 * postEngagement * viralMultiplier) + Math.floor(Math.random() * 10),
        shares: Math.floor(platform.followers * 0.02 * postEngagement * viralMultiplier) + Math.floor(Math.random() * 5),
        views: Math.floor(platform.followers * 0.5 * postEngagement * viralMultiplier) + 50 + Math.floor(Math.random() * 100),
        ...
        username: character?.artistName || "You",
        handle: character?.artistName?.toLowerCase().replace(/\s+/g, '') || "you",
        verified: currentState.stats.reputation > 70
      };
      
      // Add image(s) if applicable
      if (images && images.length > 0) {
        post.image = images[0]; // Primary image
        post.images = images; // All images
      }
      
      // For TikTok, add additional video properties
      if (platformName === "TikTok") {
        post.musicTitle = `Original Sound - ${character.artistName}`;
        post.duration = 15 + Math.floor(Math.random() * 45); // 15-60 second video
        post.bookmarks = Math.floor(post.likes * 0.3);
      }
      
      // Add post to platform stats
      get().addSocialMediaPost(platformName, post);
      
      // Update old social media structures for backward compatibility
      const oldState = get();
      
      // Find the platform in the social media array
      const oldPlatformIndex = oldState.socialMedia.findIndex(p => p.name === platformName);
      if (oldPlatformIndex !== -1) {
        // Update the platform post count and follower count
        const updatedSocialMedia = [...oldState.socialMedia];
        const platform = updatedSocialMedia[oldPlatformIndex];
        updatedSocialMedia[oldPlatformIndex] = {
          ...platform,
          posts: platform.posts + 1,
          followers: platform.followers + (post.followerGain || 0),
          lastPostWeek: oldState.currentWeek
        };
        
        // Update the state
        set({ socialMedia: updatedSocialMedia });
      }
    },
    ...
    addSocialMediaPost: (platformName, post) => {
      ...
      if (!currentState.socialMediaStats) {
        console.error("socialMediaStats is undefined or null. Initializing it.");
        ...
        return; // Return here and let the next call handle the post
      }
      
      console.log("Adding post to platform:", platformName);
      console.log("Follower gain amount:", post.followerGain);
      console.log("Current social media stats:", JSON.stringify(currentState.socialMediaStats, null, 2));
      
      const updatedStats = { ...currentState.socialMediaStats };
      
      // Twitter post
      if (lowerCasePlatform === 'twitter' && updatedStats.twitter) {
        const currentFollowers = updatedStats.twitter.followers || 50;
        const followerGain = post.followerGain || 0;
        console.log(`Twitter followers: ${currentFollowers} + ${followerGain}`);
        
        updatedStats.twitter = {
          ...updatedStats.twitter,
          tweets: [...(updatedStats.twitter.tweets || []), post],
          followers: currentFollowers + followerGain
        };
        console.log("New Twitter followers:", updatedStats.twitter.followers);
      } 
      // Instagram post
      else if (lowerCasePlatform === 'instagram' && updatedStats.instagram) {
        const currentFollowers = updatedStats.instagram.followers || 30;
        const followerGain = post.followerGain || 0;
        console.log(`Instagram followers: ${currentFollowers} + ${followerGain}`);
        ...
        console.log("New Instagram followers:", updatedStats.instagram.followers);
      } 
      // TikTok post (video)
      else if (lowerCasePlatform === 'tiktok' && updatedStats.tiktok) {
        const currentFollowers = updatedStats.tiktok.followers || 20;
        const followerGain = post.followerGain || 0;
        console.log(`TikTok followers: ${currentFollowers} + ${followerGain}`);
        ...
        console.log("New TikTok followers:", updatedStats.tiktok.followers);
      }
      
      // Update state with new social media stats
      set({ socialMediaStats: updatedStats });
      console.log("Updated social media stats:", JSON.stringify(updatedStats, null, 2));
    ...
    _oldPostOnSocialMedia: (platformName, content = "", images = []) => {
      ...
      const baseFollowerGain = Math.max(5, Math.floor(likes * 0.1)); // Base follower gain with minimum of 5
      const followerGain = Math.floor(baseFollowerGain * viralMultiplier);
      
      // Calculate reputation gain
      const baseReputationGain = Math.max(1, Math.floor(shares * 0.05)); // Minimum 1 reputation 
