...
export type GameScreen = 
  ...
  | "jobs" // Jobs screen for earning money

// Appearance customization types
export type AppearanceStyle = 'casual' | 'urban' | 'formal' | 'experimental' | 'retro' | 'modern';
export type FashionStyle = 'streetwear' | 'high-fashion' | 'vintage' | 'minimal' | 'eccentric';
export type HairStyle = 'short' | 'medium' | 'long' | 'afro' | 'braids' | 'dreadlocks' | 'bald' | 'dyed' | 'fade';
export type AccessoryType = 'chains' | 'earrings' | 'glasses' | 'grillz' | 'hats' | 'tattoos' | 'watches';
export type MusicStyle = 'trap' | 'boom bap' | 'melodic' | 'drill' | 'experimental' | 'old school' | 'pop rap' | 'lyrical';
...
export interface CharacterAppearance {
  ...
  signature?: string; // Signature look or item
  style?: AppearanceStyle; // Alias for appearanceStyle
  fashion?: FashionStyle; // Alias for fashionStyle
  hair?: HairStyle; // Alias for hairStyle
  colorScheme?: string; // Color scheme for the character
}
...
export interface CharacterInfo {
  ...
  about?: string;
  bio?: string; // Alternative to about for consistency
  musicStyle?: string;
  hometown?: string;
  background?: string;
  fanbaseName?: string; // Name for the artist's fandom
  wealth?: number; // Character's money
  appearance?: CharacterAppearance;
  country?: string; // Country of origin
  careerStartWeek?: number; // Week when career started
}
...
export interface StreamingPlatform {
  ...
  name: string;
  listeners: number;
  monthlyListeners?: number; // Monthly active listeners
  totalStreams: number;
  streams?: number; // Alias for totalStreams for consistency
  revenue: number;
...
export interface AIRapper {
  ...
  // Extended properties for artist profiles feature
  musicStyle?: string;
  description?: string;
  debutWeek?: number;
  fanbase?: number;
