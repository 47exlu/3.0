...
import { CharacterInfo, PlayerStats } from '@/lib/types';

// Helper function to convert game week to release year
const getReleaseYear = (releaseWeek: number | undefined) => {
  if (!releaseWeek) return 'Unknown';
...
export function PlayerWikipedia() {
  ...
  const { 
    character = {} as unknown as CharacterInfo, 
    currentWeek = 1, 
                      ...
                      <div key={song.id} className="flex items-center p-2 bg-gray-700 bg-opacity-30 rounded">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-400">
                            Released in {getReleaseYear(song.releaseDate)} • {formatNumber(typeof song.streams === 'number' ? song.streams : 0)} streams
                          </div>
                        </div>
                      </div>
                    ))
                  ...
                  {albums.filter(album => album.released).map(album => (
                    <div key={album.id} className="flex bg-gray-700 bg-opacity-40 rounded overflow-hidden border border-gray-600">
                      ...
                      <div className="p-3">
                        ...
                        <div className="text-xs text-gray-400 mt-1">Released in {getReleaseYear(album.releaseDate)}</div>
                    ...
                    <tbody>
                      {safeSongs
                        ...
                        <tr key={song.id} className="border-b border-gray-700 hover:bg-gray-700 hover:bg-opacity-30">
                          ...
                          <td className="py-3 px-2 text-sm">Released in {getReleaseYear(song.releaseDate)}</td>
                          <td className="py-3 px-2">
                ...
                {streamingPlatforms && streamingPlatforms.length > 0 ? (
                  <div className="space-y-3">
                    {streamingPlatforms.map(platform => {
                      const platformStreams = platform.streams || platform.totalStreams || 0;
                      const percentage = totalStreams > 0 
                        ? Math.round((platformStreams / totalStreams) * 100) 
                        : 0;
                      
                      return (
                        <div key={platform.id || platform.name} className="space-y-1">
                          <div className="flex justify-between text-sm">
                            <span>{platform.name}</span>
                            <span className="text-gray-400">{formatNumber(platformStreams)} ({percentage}%)</span>
                          </div>
