...
// AI Rapper Profiles component
const AIRapperProfiles = () => {
  const { aiRappers, songs, streamingPlatforms, character, albums } = useRapperGame();
  ...
  const [showRapperDetail, setShowRapperDetail] = useState(false);
  const [showAllSongs, setShowAllSongs] = useState(false);
  
  // Create a list of unique genres from the rappers
  const genres = useMemo(() => {
    const allGenres = aiRappers?.map(rapper => rapper.musicStyle) || [];
    return Array.from(new Set(allGenres)).filter(Boolean).sort();
  }, [aiRappers]);
  
  // Filter and sort rappers
  const filteredRappers = useMemo(() => {
    if (!aiRappers?.length) return [];
    
    let filtered = [...aiRappers];
    
    // Filter by search query
    if (searchQuery) {
      const query = searchQuery.toLowerCase();
      filtered = filtered.filter(rapper => 
        rapper.name.toLowerCase().includes(query) || 
        rapper.description?.toLowerCase().includes(query) ||
        rapper.musicStyle?.toLowerCase().includes(query)
      );
    }
    
    // Filter by genre
    if (selectedGenre) {
      filtered = filtered.filter(rapper => rapper.musicStyle === selectedGenre);
    }
    ...
    switch (sortOption) {
      ...
      case "recent":
        return filtered.sort((a, b) => (b.id ? parseInt(b.id) : 0) - (a.id ? parseInt(a.id) : 0));
      case "monthly_listeners":
        return filtered.sort((a, b) => (b.monthlyListeners || 0) - (a.monthlyListeners || 0));
      case "total_streams":
        return filtered.sort((a, b) => (b.totalStreams || 0) - (a.totalStreams || 0));
      case "rating":
        return filtered.sort((a, b) => {
          const ratingA = a.rating || Math.floor(a.popularity / 10) || 0;
          const ratingB = b.rating || Math.floor(b.popularity / 10) || 0;
          return ratingB - ratingA;
        });
      case "tier":
        return filtered.sort((a, b) => {
          const tierA = a.tier || Math.max(1, Math.floor(a.popularity / 20)) || 1;
          const tierB = b.tier || Math.max(1, Math.floor(b.popularity / 20)) || 1;
          return tierB - tierA;
        });
      default:
        return filtered;
    }
  }, [aiRappers, searchQuery, selectedGenre, sortOption]);
  ...
  // Get the total number of collaborations with a specific rapper
  const getCollaborationsCount = (rapperId: string) => {
    return songs.filter(song => 
      song.featuring && song.featuring.includes(rapperId)
    ).length;
  };
  
  // Get rapper's albums
  const getRapperAlbums = (rapperId: string) => {
    return albums?.filter(album => album.artistId === rapperId) || [];
  };
  
  // Get rapper's songs
  const getRapperSongs = (rapperId: string) => {
    return songs.filter(song => 
      song.aiRapperOwner === rapperId || 
      (song.featuring && song.featuring.includes(rapperId))
    );
  };
  
  // Show rapper detail popup
  const openRapperDetail = (rapperId: string) => {
    setSelectedRapper(rapperId);
    setShowRapperDetail(true);
    setShowAllSongs(false); // Reset songs view when opening a new rapper
  };
  
  // Format relationship status
  const formatRelationship = (status?: string) => {
    switch (status) {
  ...
  // Selected rapper for detail view
  const currentRapper = useMemo(() => {
    if (!selectedRapper) return null;
    return aiRappers.find(r => r.id === selectedRapper);
  }, [selectedRapper, aiRappers]);
  ...
  return (
    <div className="p-4 sm:p-6 text-white">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="bg-green-950 text-white border border-green-800 rounded-full py-1.5 pl-9 pr-4 w-full text-sm focus:outline-none focus:ring-1 focus:ring-green-600"
              value={searchQuery}
          ...
          {/* Genre filter */}
          <select
            className="bg-green-950 text-white border border-green-800 rounded-full py-1.5 px-4 text-sm focus:outline-none focus:ring-1 focus:ring-green-600"
            value={selectedGenre || ""}
          ...
          >
            ...
            <option value="monthly_listeners">Sort by Monthly Listeners</option>
            <option value="total_streams">Sort by Total Streams</option>
      ...
      {filteredRappers.length === 0 ? (
        <div className="text-center py-10 bg-green-950 rounded-lg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {filteredRappers.map(rapper => (
            <div 
              key={rapper.id} 
              className="bg-green-950 border border-green-900 rounded-lg overflow-hidden hover:bg-green-900 transition-colors cursor-pointer"
              onClick={() => openRapperDetail(rapper.id)}
            >
              <div className="flex p-4">
                {/* Rapper image */}
                <div className="h-24 w-24 rounded-md overflow-hidden flex-shrink-0 mr-4 bg-green-900">
                  {rapper.image ? (
                    <img src={rapper.image} alt={rapper.name} className="h-full w-full object-cover" />
                  ) : (
                    <div className="h-full w-full flex items-center justify-center bg-gradient-to-br from-green-800 to-green-950">
                      <svg className="h-12 w-12 text-green-700" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center">
                    <h3 className="font-bold text-lg truncate">{rapper.name}</h3>
                    {character && rapper.name === character.artistName && (
                      <Badge className="ml-2 bg-blue-600 text-[10px] py-0">You</Badge>
                    )}
                  </div>
                  <div className="text-green-400 text-xs">{rapper.musicStyle || "Hip Hop"}</div>
                  ...
                  <div className="mt-2 text-xs text-gray-300 space-y-1">
                    <div className="flex items-center">
                      ...
                      <span>Debut: Week {rapper.debutWeek || 1}</span>
                    </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <span>{formatNumber(rapper.fanbase || 0)} fans</span>
                    </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <span>{formatNumber(rapper.monthlyListeners || 0)} monthly</span>
                    </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <span>{formatNumber(rapper.totalStreams || 0)} total streams</span>
                    </div>
                    ...
                    <div className="flex items-center">
                      ...
                      <span>{getCollaborationsCount(rapper.id)} collaborations</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Quick stats */}
              <div className="border-t border-green-800 px-4 py-2 flex items-center justify-between text-xs text-gray-300">
                <div className="flex items-center">
                  <svg className="h-3 w-3 mr-1 text-yellow-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                    <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                  </svg>
                  <span>{rapper.rating || Math.floor(Math.random() * 3) + 8}/10 Rating</span>
                </div>
                
                <Separator orientation="vertical" className="h-4 bg-green-800" />
                
                <div>
                  <span className="font-semibold">{Math.max(1, rapper.tier || 1)}</span> Tier Artist
                </div>
      ...
      {/* Rapper Detail Dialog */}
      <Dialog 
        open={showRapperDetail} 
        onOpenChange={(open) => {
          setShowRapperDetail(open);
          if (!open) setShowAllSongs(false); // Reset songs view when closing
        }}>
        <DialogContent className="sm:max-w-[800px] bg-gradient-to-b from-green-900 to-black text-white border-0 p-0 max-h-[90vh] overflow-y-auto" aria-describedby="rapper-profile-description">
          <DialogTitle className="sr-only">Artist Profile</DialogTitle>
          <DialogDescription id="rapper-profile-description" className="sr-only">
            Detailed information about this artist, including their stats, songs, and albums.
          </DialogDescription>
          {currentRapper && (
            <div className="relative">
              {/* Cover Background */}
              <div className="w-full h-40 bg-gradient-to-r from-green-900 to-black overflow-hidden">
                {currentRapper.image && (
                  <div 
                    className="w-full h-full blur-md opacity-40" 
                    style={{ backgroundImage: `url(${currentRapper.image})`, backgroundSize: 'cover', backgroundPosition: 'center' }}
                  />
                )}
              </div>
              ...
              <button 
                ...
                onClick={() => setShowRapperDetail(false)}
              >
              ...
              {/* Artist Info */}
              <div className="flex px-6 -mt-16 relative z-10">
                {/* Artist image */}
                <div className="h-32 w-32 rounded-full overflow-hidden border-4 border-black flex-shrink-0">
                  {currentRapper.image ? (
                    <img src={currentRapper.image} alt={currentRapper.name} className="h-full w-full object-cover" />
                  ) : (
                    <div className="h-full w-full bg-gradient-to-br from-green-700 to-green-950 flex items-center justify-center">
                      <svg className="h-16 w-16 text-green-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path>
                        <circle cx="12" cy="7" r="4"></circle>
                      </svg>
                    </div>
                ...
                <div className="mt-16 ml-4">
                  <div className="flex items-center">
                    <h2 className="text-2xl font-bold">{currentRapper.name}</h2>
                    {character && currentRapper.name === character.artistName && (
                      <Badge className="ml-3 bg-blue-600">You</Badge>
                    )}
                  </div>
                  <div className="text-green-400">{currentRapper.musicStyle || "Hip Hop"}</div>
                  <div className="flex items-center mt-1 text-sm text-gray-300">
                    <span>
                      <span className="font-semibold">{formatNumber(currentRapper.fanbase || 0)}</span> fans
                    </span>
                    <span className="mx-2">•</span>
                    <span>{formatRelationship(currentRapper.relationshipStatus || currentRapper.relationship)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <p className="text-gray-300 text-sm">
                    {currentRapper.description || currentRapper.bio || 
                     `${currentRapper.name} is a ${currentRapper.musicStyle || 'hip hop'} artist with a unique style and growing following.`}
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    <div className="bg-green-950 rounded-lg p-3">
                      ...
                      <div className="text-xl font-semibold">{formatNumber(currentRapper.monthlyListeners || 0)}</div>
                    </div>
                    <div className="bg-green-950 rounded-lg p-3">
                      <div className="text-xs text-gray-400">Total Streams</div>
                      <div className="text-xl font-semibold">{formatNumber(currentRapper.totalStreams || 0)}</div>
                    </div>
                    <div className="bg-green-950 rounded-lg p-3">
                      <div className="text-xs text-gray-400">Followers</div>
                      <div className="text-xl font-semibold">{formatNumber(currentRapper.fanbase || 0)}</div>
                    </div>
                    <div className="bg-green-950 rounded-lg p-3">
                      <div className="text-xs text-gray-400">Popularity</div>
                      <div className="text-xl font-semibold">{currentRapper.popularity || 0}</div>
                      <div className="w-full h-1.5 bg-gray-700 rounded-full mt-1 overflow-hidden">
                        <div 
                          className="h-full bg-green-500 rounded-full" 
                          style={{ width: `${currentRapper.popularity || 0}%` }}
                        ></div>
                      </div>
                    </div>
                    <div className="bg-green-950 rounded-lg p-3">
                      <div className="text-xs text-gray-400">Tier</div>
                      <div className="text-xl font-semibold">{currentRapper.tier || Math.max(1, Math.floor(currentRapper.popularity / 20))}</div>
                    </div>
                    <div className="bg-green-950 rounded-lg p-3">
                      <div className="text-xs text-gray-400">Rating</div>
                      <div className="text-xl font-semibold flex items-center">
                        {currentRapper.rating || Math.floor(currentRapper.popularity / 10)}/10
                        <svg className="h-4 w-4 ml-1 text-yellow-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                          <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                        </svg>
                      </div>
                    </div>
                    <div className="bg-green-950 rounded-lg p-3">
                      <div className="text-xs text-gray-400">Feature Cost</div>
                      <div className="text-xl font-semibold">${formatNumber(currentRapper.featureCost || 0)}</div>
                    </div>
                    <div className="bg-green-950 rounded-lg p-3">
                      <div className="text-xs text-gray-400">Debut Week</div>
                      <div className="text-xl font-semibold">Week {currentRapper.debutWeek || 1}</div>
                    </div>
                  </div>
                </div>
                
                {/* Albums */}
                {getRapperAlbums(currentRapper.id).length > 0 && (
                  <div>
                    <h3 className="text-lg font-semibold mb-3">Albums</h3>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {getRapperAlbums(currentRapper.id).map(album => (
                        <div key={album.id} className="bg-green-950 rounded-lg overflow-hidden">
                          <div className="w-full aspect-square">
                            {album.coverArt ? (
                              <img src={album.coverArt} alt={album.title} className="w-full h-full object-cover" />
                            ) : (
                              <div className="w-full h-full bg-gradient-to-br from-green-700 to-black flex items-center justify-center">
                                <svg className="h-12 w-12 text-green-600" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round">
                                  <path d="M9 18V5l12-2v13"></path>
                                  <circle cx="6" cy="18" r="3"></circle>
                                  <circle cx="18" cy="16" r="3"></circle>
                                </svg>
                          ...
                          <div className="p-2">
                            ...
                            <div className="text-xs text-gray-400">{album.type || 'Album'}</div>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {/* Top Tracks */}
                {getRapperSongs(currentRapper.id).length > 0 && (
                  <div>
                    <h3 className="text-lg font-semibold mb-3">Top Tracks</h3>
                    <div className="space-y-2">
                      {getRapperSongs(currentRapper.id)
                        .sort((a, b) => (b.streams || 0) - (a.streams || 0))
                        .slice(0, 5)
                        .map((song, idx) => (
                          <div key={song.id} className="flex items-center p-2 hover:bg-green-950 rounded-md">
                            <div className="w-6 text-center text-gray-500">{idx + 1}</div>
                            <div className="ml-3 flex-grow">
                              <div className="font-medium">{song.title}</div>
                              <div className="text-xs text-gray-400">
                                {song.featuring.length > 0 && `ft. ${song.featuring.map(fId => {
                                  const artist = aiRappers.find(r => r.id === fId);
                                  return artist ? artist.name : '';
                                }).filter(Boolean).join(', ')}`}
                              </div>
                            </div>
                            <div className="text-sm text-gray-400">{formatNumber(song.streams || 0)}</div>
                          </div>
                        ))}
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap gap-3 pt-2">
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      // Toggle a "view all songs" section within the artist profile
                      setShowAllSongs(!showAllSongs);
                    }}
                  >
                    {showAllSongs ? "Hide Songs" : "View All Songs"}
                  </Button>
                </div>
                
                {/* All Songs Section (conditionally displayed) */}
                {showAllSongs && (
                  <div className="mt-6 border-t border-gray-800 pt-6">
                    <h3 className="text-lg font-semibold mb-3">All Songs</h3>
                    {getRapperSongs(currentRapper.id).length === 0 ? (
                      <div className="py-8 text-center text-gray-400 bg-green-950/50 rounded-lg">
                      ...
                      <div className="space-y-1.5">
                        {getRapperSongs(currentRapper.id)
                          .sort((a, b) => (b.streams || 0) - (a.streams || 0))
                          .map((song, idx) => (
                            <div key={song.id} className="flex items-center p-3 hover:bg-green-900/50 rounded-md group">
                              <div className="w-5 text-center text-gray-500">{idx + 1}</div>
                              
                              {/* Song cover image */}
                              <div className="h-10 w-10 ml-3 rounded overflow-hidden">
                                {song.coverArt ? (
                                  <img src={song.coverArt} alt={song.title} className="h-full w-full object-cover" />
                                ) : (
                                  <div className="h-full w-full bg-green-800 flex items-center justify-center">
                              ...
                              <div className="flex items-center gap-4">
                                <div className="text-sm text-gray-400">{formatNumber(song.streams || 0)} streams</div>
                                <div className="text-xs text-gray-500">
                                  {song.releaseDate ? `Week ${song.releaseDate}` : 'Unreleased'}
                                </div>
                                {song.performanceType !== "normal" && (
                                  <Badge className={`${
                                    song.performanceType === "viral" ? "bg-green-600" : 
                                    song.performanceType === "flop" ? "bg-red-700" : 
                                    "bg-blue-600"
                                  } text-[10px]`}>
                                    {song.performanceType === "viral" ? "Viral" : 
                                     song.performanceType === "flop" ? "Flop" : 
                                     "Comeback"}
                                  </Badge>
                                )}
                              </div>
...
export function StreamingPlatforms() {
  const { streamingPlatforms, videosPlatforms, songs, stats, character, setScreen, updateCharacter, musicVideos, albums, aiRappers } = useRapperGame();
      ...
      >
        <TabsList className="bg-green-950 border border-green-800 w-full flex flex-wrap overflow-x-auto h-auto">
          ...
          <TabsTrigger value="artists" className="data-[state=active]:bg-green-800 relative flex-1 text-[10px] xxs:text-[11px] xs:text-xs sm:text-sm py-1 px-1 sm:px-2 h-auto">
            <svg className="inline-block mr-0.5 sm:mr-1 h-3 w-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"></path>
              <circle cx="9" cy="7" r="4"></circle>
              <path d="M22 21v-2a4 4 0 0 0-3-3.87"></path>
              <path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
                                  ...
                                  <div key={song.id} className="flex items-center px-2 xs:px-3 sm:px-5 py-2 xs:py-3 hover:bg-[#282828] rounded-md group">
                                    ...
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <div className="text-[10px] xxs:text-xs text-[#b3b3b3] truncate">
                                        {song.featuring.length > 0 ? `Feat. ${song.featuring.join(', ')}` : 'Single'}
                                      </div>
                                    </div>
                                  ...
                                  <div className="text-[10px] xxs:text-xs text-[#b3b3b3] mt-1 xxs:mt-2 line-clamp-2">
                                    {selectedPlatform === 'spotify' && spotify ? 
                                     ...
                                     selectedPlatform === 'soundcloud' && soundCloud ?
                                       formatNumber(Math.floor(song.streams * (0.05 * (0.9 + Math.random() * 0.2)))) :
                                       formatNumber(song.streams)
                                    } streams • Feat. {song.featuring.join(', ')}
                                  </div>
                                </div>
                              ))}
                                  ...
                                  <div className="flex items-center">
                                    ...
                                    {song.featuring && song.featuring.length > 0 && (
                                      <Badge variant="outline" className="text-[10px] h-4 px-1 border-gray-300 text-gray-500">
                                        ft. {song.featuring.map(featuringId => featuringId).join(', ')}
                                      </Badge>
                                    )}
                                  </div>
                                      ...
                                      <div>
                                        ...
                                        <div className="text-xs text-gray-500">
                                          {song.featuring.length > 0 ? 
                                            `Feat. ${song.featuring.map(featuringId => featuringId).join(', ')}` : 
                                            'Single'}
                                        </div>
                                      </div>
                                    </div>
